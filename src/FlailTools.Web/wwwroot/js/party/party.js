...
// its own — no dice, no FLAIL!, no wording — which is what lets the interesting half stay testable
...
// Namespaces the signalling so two unrelated apps using the same relays never meet, even if
// somebody picks the same room code by accident.
const appId = 'structed-flail-tools-dice';
...
export async function join(code, handler) {
    leave();
