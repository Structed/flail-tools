...
/// <remarks>
/// <para>
/// Deliberately <em>not</em> the engine's <c>DiceExpression</c>, which reads what a table says —
/// <c>d6 x 10p</c> — and answers with a total. This one is typed by a person into a box at the
/// table, has to keep every individual face so the party can see the dice, and has to be able to
/// refuse politely rather than throw: somebody will type <c>2d</c> mid-thought and the page must not
/// fall over. The two are different jobs and they will have to coexist, so they do not share a name.
/// </para>
/// <para>
